...
class Con_Proximity(nn.Module):
    ...
    def __init__(self, num_classes, feat_dim, CUDA_num, use_gpu=True):
        ...
        if self.use_gpu:
            self.centers = nn.Parameter(torch.randn(self.num_classes, self.feat_dim).cuda(self.CUDA_num))#100 x feats- for 100 centers
        else:
    ...
    def forward(self, x, labels):
        ...
        if self.use_gpu: classes = classes.cuda(self.CUDA_num)
