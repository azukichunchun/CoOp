...
class CustomCLIP(nn.Module):
    ...
    def __init__(self, cfg, classnames, clip_model, category_feature_stats, augment_num, augment_text_length, tfms_num):
        super().__init__()
        ...
        self.category_feature_stats = category_feature_stats
    ...
    def augment_imfs_from_gaussian(self, image_features, label):

        img_aug = []
        #cov = torch.eye(512, device=image_features[0].device)
        #pdb.set_trace()
        for imf, lbl in zip(image_features, label):
            category_name = self.classnames[lbl]
            cov = self.category_feature_stats[category_name][1] # cov
            cov = cov.to(image_features[0].device)
            mvn = torch.distributions.MultivariateNormal(imf, cov)
            img_aug.extend([mvn.sample((self.augment_num,))])
        return torch.stack(img_aug).reshape(-1, 512).to(imf.device)
    ...
    def forward(self, image, label=None):
        ...
        if self.save == True:
            with open(osp.join(self.cfg.OUTPUT_DIR, "img_features_before_aug.pkl"), "wb") as f:
                pickle.dump(image_features, f)
            #self.save = False

        if self.augment_num > 0:
            image_features = self.augment_imfs_from_gaussian(image_features, label)
    ...
    def inference(self, image):
        image_features = self.image_encoder(image.type(self.dtype))
        image_features_original = copy.deepcopy(image_features)
        ...
        return logits, image_features_original


@TRAINER_REGISTRY.register()
class OneShot_Adapter_Diverse(TrainerX):
    """ CLIP-Adapter """
    ...
    def build_model(self):
        ...
        # Calulate templeate mean and sigma
        clip_model.to(self.device)
        epsilon = 1e-6
        self.category_feature_stats = dict()
        for i, c in enumerate(classnames):
            prompts = [temp.format(c.replace("_", " ")) for temp in self.templates]
            prompts = torch.cat([clip.tokenize(p) for p in prompts]).to(self.device)

            with torch.no_grad():
                text_features = clip_model.encode_text(prompts).detach().cpu()
            
            mu = torch.mean(text_features.detach().cpu(), dim=0)
            cov_ = torch.cov(text_features.detach().cpu().T)
            cov = cov_ @ cov_.T + epsilon * torch.eye(512)

            self.category_feature_stats[c] = [mu, cov]

       
        print('Building custom CLIP')
        self.model = CustomCLIP(cfg, classnames, clip_model, self.category_feature_stats, 
                                augment_num=100, augment_text_length=3, tfms_num=3)
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output, embedding = self.model_inference(input)
            self.evaluator.process(output, label)

            embeddings.append(embedding)
        
        embeddings = torch.stack(embeddings)
        with open(osp.join(self.cfg.OUTPUT_DIR, "embeddings_test_data.pkl"), "wb") as f:
