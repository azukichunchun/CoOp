...
class TextEncoder(nn.Module):
    ...
    def forward(self, augment_num=0, augment_length=3):
        ...
        if augment_num > 0: # ["a", "b", "c"] -> ["a1", "a2", "a3", "b1", "b2", "b3", "c1", "c2", "c3"]
            prompts_aug = []
            for p in prompts:
                prompt_aug_ = []
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        # 言語特徴量のデータ拡張→オフにする
        augment_num = 0
        augment_text_length = 0
        text_features = self.text_encoder(augment_num, augment_text_length)
...
class OneShot_Adapter_Augimg(TrainerX):
    """ CLIP-Adapter """
